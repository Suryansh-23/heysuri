...
Input:
  t*          : target timestamp
  b_min, b_max: inclusive block bounds (from checkpoints / earliest / latest)
  TS(b)       : function that returns block b's timestamp (RPC)
Output:
  b_hat       : block number closest to t*
...
<SearchVisualizer
  ...
  defaults={{ slopeInterval: 4, duplicateProbability: 0.18, duplicateLength: 20 }}
/>
